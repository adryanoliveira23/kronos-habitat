...
import { Eye, EyeOff, Zap, ArrowLeft, Mail, Lock, User, ArrowRight } from "lucide-react";
...
export default function RegisterPage({ onBack, onGoToLogin, selectedPlan }: RegisterPageProps) {
    const [showPassword, setShowPassword] = useState(false);
    const [name, setName] = useState("");
    const [email, setEmail] = useState("");
    const [password, setPassword] = useState("");

    const planLabel = selectedPlan === "pro" ? "Kronos Habit PRO" : "Kronos Habit";
    const planColor = selectedPlan === "pro" ? "text-neon-green" : "text-neon-yellow";
    const planBg = selectedPlan === "pro" ? "bg-neon-green/10 border-neon-green/20" : "bg-neon-yellow/10 border-neon-yellow/20";
    const planTextColor = selectedPlan === "pro" ? "text-neon-green" : "text-neon-yellow";
                ...
                >
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="mb-8">
                            ...
                            {selectedPlan && (
                                <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border mb-4 ${planBg}`}>
                                    <Zap className={`w-3 h-3 fill-current ${planTextColor}`} />
                                    <span className={`text-[10px] font-black uppercase tracking-widest ${planTextColor}`}>{planLabel}</span>
                                </div>
                            )}
                            {!selectedPlan && (
                                <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-neon-yellow/10 border border-neon-yellow/20 rounded-full mb-4">
                                    ...
                                    <span className="text-neon-yellow text-[10px] font-black uppercase tracking-widest">Criar Conta</span>
                                </div>
                            )}
                        ...
                        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                            ...
                            <motion.button
                                ...
                                whileTap={{ scale: 0.98 }}
                                className="w-full py-4 bg-neon-yellow text-black font-black text-sm uppercase tracking-widest rounded-2xl shadow-[0_0_20px_rgba(232,255,0,0.2)] hover:shadow-[0_0_30px_rgba(232,255,0,0.4)] transition-all flex items-center justify-center gap-2 mt-2"
                            >
                        ...
                        <div className="flex items-center gap-4 my-6">
                            ...
                            <span className="text-white/30 text-[10px] uppercase font-bold tracking-widest">ou</span>
                            <div className="flex-1 h-px bg-white/10" />
                        </div>
                        ...
                        <p className="text-center text-white/40 text-sm">
                            Já tem conta?{" "}
                            <button onClick={onGoToLogin} className="text-neon-green font-black hover:underline transition-all">
                                Fazer login
                            </button>
                        </p>
                    </div>
