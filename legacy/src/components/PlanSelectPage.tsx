...
export default function PlanSelectPage({ onBack, onSelectPlan }: PlanSelectPageProps) {
    const plans = [
        {
            id: "basic" as const,
            name: "Kronos Habit",
            ...
            features: ["Hábitos ilimitados", "Missões diárias", "Sistema de XP e Level", "Dashboard gamificado", "Ranking global"],
            tag: null,
            badgeName: "Iniciante",
            period: "/mês"
        },
        {
            ...
            name: "Kronos Premium",
            price: "22,08",
            color: "border-neon-green shadow-[0_0_40px_rgba(62,255,139,0.15)] scale-105 z-10",
            accentColor: "text-neon-green",
            btnClass: "bg-neon-green text-black hover:bg-neon-green/90",
            features: ["Tudo do plano PRO", "Acesso total a todos os recursos", "Mentoria com IA Ilimitada", "Personalização Avançada", "Selo Exclusivo de Pioneiro"],
            tag: "Promoção Anual",
            badgeName: "Lendário",
            period: "/mês",
            strikePrice: "59,90",
        ...
        },
        {
            ...
            price: "32,90",
            color: "border-blue-400/50 shadow-[0_0_30px_rgba(96,165,250,0.1)] hover:border-blue-400",
            accentColor: "text-blue-400",
            glowColor: "",
            btnClass: "bg-blue-500/10 text-blue-400 hover:bg-blue-500 hover:text-white border border-blue-500/30",
            features: ["IA avançada (Mentor estratégico)", "Sistema multiplayer completo", "Desafios privados", "Análises profundas", "Status Elite na comunidade"],
            tag: "Popular",
            badgeName: "Elite",
            period: "/mês"
        },
    ...
    return (
        <div className="min-h-screen bg-bg-dark flex items-center justify-center relative overflow-hidden px-4 py-16">
            ...
            <div className="relative z-10 w-full max-w-[1000px]">
                ...
                >
                    <p className="text-neon-green text-[10px] font-black uppercase tracking-[0.4em] mb-3">Escolha seu nível</p>
                    ...
                    <p className="text-white/40 text-sm mt-4 font-medium max-w-[500px] mx-auto">
                        Selecione o plano que melhor representa sua ambição. Você pode fazer upgrade a qualquer momento.
                    </p>
                </motion.div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {plans.map((plan, i) => (
                        <motion.div
                            ...
                            onClick={() => onSelectPlan(plan.id === "powerful" ? "pro" : plan.id)}
                        >
                            {/* Popular tag */}
                            {plan.tag && (
                                <div className={`absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1.5 text-[9px] font-black uppercase tracking-widest rounded-full whitespace-nowrap ${plan.id === 'powerful' ? "bg-neon-green text-black" : "bg-blue-500 text-white"}`}>
                                    {plan.tag}
                                </div>
                            )}
                            ...
                            {/* Plan name */}
                            <div className="mb-6">
                                <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border ${plan.id === "powerful" ? "bg-neon-green/10 border-neon-green/20" : plan.id === "pro" ? "bg-blue-500/10 border-blue-500/20" : "bg-neon-yellow/10 border-neon-yellow/20"} mb-4`}>
                                    <Zap className={`w-3 h-3 fill-current ${plan.accentColor}`} />
                                    <span className={`text-[10px] font-black uppercase tracking-widest ${plan.accentColor}`}>{plan.badgeName}</span>
                                </div>
                                ...
                                <div className="flex items-baseline gap-1 mt-1">
                                    ...
                                    <span className="text-[11px] text-white/40 font-bold">{plan.period || "/mês"}</span>
                                </div>
                            ...
                            <ul className="space-y-3 mb-8">
                                {plan.features.map((feat, j) => (
                                    <li key={j} className="flex items-start gap-3 text-[11px] font-bold uppercase tracking-tight text-white/70">
                            ...
                            {/* CTA Button */}
                            <button
                                onClick={(e) => { e.stopPropagation(); onSelectPlan(plan.id === "powerful" ? "pro" : plan.id); }}
